<div class="relative">
    @if ('textarea' != $type)
        ...
        <input x-ref="input-{{ $name }}" type="{{ $type }}" name="{{ $name }}"
            placeholder="{{ $placeholder }}" value="{{ old($name, $value) }}" id="{{ $name }}"
            @class([
                'w-full rounded-md border-0 py-1.5 text-sm ring-1 ring-slate-300 pr-4 placeholder:text-slate-400 focus:ring-2',
                'pr-8' => $formRef,
