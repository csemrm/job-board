    ...
    <x-card>
        ...
        <form action="{{ route('job.application.store', $job) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <x-text-input type="number" required id="expected_salary" name="expected_salary" />
            </div>
            <div class="mb-4">
                <x-label for="cv" :required="true">Upload CV</x-label>
                <x-text-input type="file" required id="cv" name="cv" />
            </div>
            <x-button class="w-full bg-white"> Apply</x-button>
